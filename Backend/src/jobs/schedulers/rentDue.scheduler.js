...
const runRentDueJob = async () => {
	...
	for (const ledger of ledgers) {
		try {
			...
			logger.error(`Rent due+1 reminder failed for ledger ${ledger.id}: ${error.message}`);
			await prisma.notification_logs.create({
				data: {
					type: 'RENT_DUE',
					channel: 'EMAIL',
...
const startRentDueScheduler = () => {
	cron.schedule('5 9 * * *', async () => {
		logger.info('Running rent due (+1 day) scheduler...');
		await runRentDueJob();
	}, { timezone: 'Asia/Kolkata' });
};

module.exports = { startRentDueScheduler, runRentDueJob };
